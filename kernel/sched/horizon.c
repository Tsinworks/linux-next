...
static inline struct task_struct *task_of_hzn(struct sched_hzn_entity *entity)
{
...
pick_next_task_horizon(struct rq *rq)
{
	...
	if (hzn_rq->curr) {
		...
		case HZN_YIELD_NONE:
			return task_of_hzn(hzn_rq->curr);
	...
	if (hzn_rq->curr &&
		...
		hzn_rq->curr = NULL;
	return hzn_rq->curr ? task_of_hzn(hzn_rq->curr) : NULL;
}
...
#ifdef CONFIG_SMP
static int
select_task_rq_horizon(struct task_struct *p, int cpu, int sd_flag)
{
