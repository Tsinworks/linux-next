...
static void put_pi_state(struct futex_pi_state *pi_state)
{
	...
	if (pi_state->owner) {
		...
		raw_spin_lock_irqsave(&pi_state->pi_mutex.rtmutex.wait_lock, flags);
		pi_state_update_owner(pi_state, NULL);
		rt_mutex_proxy_unlock(&pi_state->pi_mutex.rtmutex);
		raw_spin_unlock_irqrestore(&pi_state->pi_mutex.rtmutex.wait_lock, flags);
	}
...
static void exit_pi_state_list(struct task_struct *curr)
{
	...
	while (!list_empty(head)) {
		...
		if (head->next != next) {
			...
			raw_spin_unlock(&pi_state->pi_mutex.rtmutex.wait_lock);
			spin_unlock(&hb->lock);
		...
		raw_spin_unlock_irq(&pi_state->pi_mutex.rtmutex.wait_lock);
		spin_unlock(&hb->lock);

		rt_mutex_futex_unlock(&pi_state->pi_mutex.rtmutex);
		put_pi_state(pi_state);
			      ...
			      struct futex_pi_state **ps)
{
	 ...
	 */
	raw_spin_lock_irq(&pi_state->pi_mutex.rtmutex.wait_lock);
			      ...
			      struct task_struct **exiting)
{
	 ...
	 * the owner of it:
	 */
	rt_mutex_init_proxy_locked(&pi_state->pi_mutex.rtmutex, p);
...
static int wake_futex_pi(u32 __user *uaddr, u32 uval, struct futex_pi_state *pi_state)
{
	...
	new_owner = rt_mutex_next_owner(&pi_state->pi_mutex.rtmutex);
	...
	if (!ret) {
		...
		postunlock = __rt_mutex_futex_unlock(&pi_state->pi_mutex.rtmutex, &wake_q);
	}
...
retry_private:
	...
	plist_for_each_entry_safe(this, next, &hb1->chain, list) {
		...
		if (requeue_pi) {
			...
			ret = rt_mutex_start_proxy_lock(&pi_state->pi_mutex.rtmutex,
							this->rt_waiter,
							this->task);
			if (ret == 1) {
				/*
...
retry:
	if (!argowner) {
		...
		if (__rt_mutex_futex_trylock(&pi_state->pi_mutex.rtmutex)) {
		 ...
		 */
		newowner = rt_mutex_owner(&pi_state->pi_mutex.rtmutex);
		/*
...
handle_err:
	 ...
	 */
	pi_state_update_owner(pi_state, rt_mutex_owner(&pi_state->pi_mutex.rtmutex));
